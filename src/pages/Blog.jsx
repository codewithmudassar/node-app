...
import {  User, Calendar, Edit, Delete, DeleteIcon, Trash } from 'lucide-react';
    ...
    <div className="bg-gray-50 min-h-screen mt-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {blog.map((post, index) => (
            <article key={index} className="bg-white rounded-lg shadow-lg overflow-hidden transition-transform duration-300 hover:transform hover:scale-105">
              <a href={`/blog/${post._id}`} className="block relative">
                <img 
                  className="w-full h-48 object-cover" 
                  src={post.img || "https://flowbite.com/docs/images/blog/image-1.jpg"} 
                  alt={post.title} 
                />
              ...
              <div className="p-6">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <a href={`/blog/update/${post._id}`}  className='text-gray-900 hover:text-red-500'><Edit className='h-4 w-4' /></a>
                  <button onClick={()=> handleDelete(post._id)} className='text-gray-900 hover:text-red-500'><Trash className='h-4 w-4' /></button>
                  </div>
                </div>
              </div>
            </article>
